...
#: Every touch/hotspot in these tests belongs to this one project; cross-project
#: isolation gets its own dedicated test.
PID = 1
...
def db(tmp_path: Path) -> Path:
    ...
    conn.execute("INSERT INTO projects (id, name, path) VALUES (?, 'p', '/p')", (PID,))
    conn.commit()
    conn.close()
    return path


async def _session(conn, external_id: str, *, project_id: int = PID) -> int:
    session = await repo.upsert_session(
        conn,
        Session(
            source="claude_code", external_id=external_id,
            project_id=project_id, content_stored=False,
        ),
    )
...
async def _touch(
    conn, session_id: int, path: str, kind: str, confidence: float,
    when: str | None = None, *, project_id: int = PID,
):
    await repo.insert_session_file_touch(
        conn,
        SessionFileTouch(
            session_id=session_id, project_id=project_id, file_path=path,
...
async def test_evidence_strength_is_the_strongest_touch_not_the_sum(db: Path):
    ...
    async with get_connection(db) as conn:
        ...
        evidence = await authorship_evidence_for(conn, ["src/a.py"], project_id=PID)
...
async def test_a_file_with_no_touches_is_absent_not_a_fabricated_zero(db: Path):
    ...
    async with get_connection(db) as conn:
        evidence = await authorship_evidence_for(conn, ["src/untouched.py"], project_id=PID)
    assert evidence == {}
...
async def test_weighting_never_exceeds_the_base_risk(db: Path):
    ...
    async with get_connection(db) as conn:
        ...
        weighted = await weight_hotspots(conn, {"src/hot.py": 80.0}, project_id=PID)
...
async def test_a_risky_file_with_no_ai_evidence_is_kept_and_weighted_to_zero(db: Path):
    ...
    async with get_connection(db) as conn:
        weighted = await weight_hotspots(conn, {"src/human.py": 90.0}, project_id=PID)
...
async def test_empty_inputs_are_handled(db: Path):
    async with get_connection(db) as conn:
        assert await authorship_evidence_for(conn, [], project_id=PID) == {}
        assert await weight_hotspots(conn, {}, project_id=PID) == []
...
async def test_a_deleted_file_is_authorship_evidence_not_silence(db: Path):
    ...
    async with get_connection(db) as conn:
        ...
        evidence = await authorship_evidence_for(conn, ["src/gone.py"], project_id=PID)
...
async def test_more_paths_than_the_sql_variable_limit(db: Path):
    ...
    async with get_connection(db) as conn:
        ...
        evidence = await authorship_evidence_for(conn, paths, project_id=PID)
...
async def test_negative_base_risk_is_refused(db: Path):
    ...
    async with get_connection(db) as conn:
        with pytest.raises(ValueError, match="non-negative"):
            await weight_hotspots(conn, {"src/a.py": -50.0}, project_id=PID)
...
async def test_a_delete_makes_a_hotspot_weight_nonzero(db: Path):
    ...
    async with get_connection(db) as conn:
        ...
        weighted = await weight_hotspots(conn, {"src/gone.py": 50.0}, project_id=PID)
    assert weighted[0].weighted_risk == 40.0
    assert weighted[0].evidence.ai_delete_touches == 1


async def test_evidence_is_scoped_to_its_project(db: Path):
    """The hole both whole-subsystem audits reproduced: a same-named file touched
    by another project's session must not leak into this project's risk."""
    async with get_connection(db) as conn:
        # project 2 exists alongside PID(=1)
        await conn.execute("INSERT INTO projects (id, name, path) VALUES (2, 'other', '/other')")
        await conn.commit()
        s_here = await _session(conn, "here", project_id=PID)
        s_other = await _session(conn, "other", project_id=2)
        await _touch(conn, s_here, "README.md", "write", 0.9, project_id=PID)
        await _touch(conn, s_other, "README.md", "write", 0.9, project_id=2)

        ev = await authorship_evidence_for(conn, ["README.md"], project_id=PID)
        assert ev["README.md"].ai_write_touches == 1, "only this project's touch, not the other's"

        w = await weight_hotspots(conn, {"README.md": 80.0}, project_id=2)
        assert w[0].evidence.ai_write_touches == 1, "project 2 sees only its own"
