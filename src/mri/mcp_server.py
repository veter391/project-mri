...
def build_server(db_path: Path | None = None) -> Any:
    ...
    async def _project_id(conn: Any, project_path: str) -> int:
        root = await _resolve(project_path)
        return await upsert_project(conn, path=str(root), name=root.name, default_branch="HEAD")
    ...
    async def fuse_project(project_path: str, top: int = 10) -> dict:
        ...
        loop. Returns per-file explanations plus a count from each stage. This is
        the heavy call; the other tools read what it stores."""
        import git

        root = await _resolve(project_path)
        async with get_connection(resolved_db) as conn:
            pid = await _project_id(conn, str(root))
            hotspots = await top_risk_files(conn, pid, limit=max(1, min(top, 100)))
            adr_dir = root / "docs" / "adr"
            report = await run_fusion(
                conn, git.Repo(root), root, project_id=pid,
                hotspots=hotspots or None, adr_dir=adr_dir if adr_dir.is_dir() else None,
            )
        return {
    ...
    async def explain_file(project_path: str, file_path: str) -> dict:
        ...
        previous `fuse_project` stored. Every clause is a stored fact; a file with
        no fusion evidence says so plainly."""
        async with get_connection(resolved_db) as conn:
            pid = await _project_id(conn, project_path)
            hotspots = await top_risk_files(conn, pid, limit=100)
        ...
        return {
            "file": exp.file_path,
            "prose": exp.prose,
            "factors": [{"name": f.name, "value": f.value} for f in exp.factors],
        }
    ...
    async def get_authorship(project_path: str, file_path: str) -> dict:
        ...
        async with get_connection(resolved_db) as conn:
            pid = await _project_id(conn, project_path)
            shares = await repo.authorship_for_file(conn, file_path, project_id=pid)
        if not shares:
            return {"file": file_path, "computed": False}
        s = shares[0]
        return {
            "file": file_path, "computed": True,
    ...
    async def get_decisions(project_path: str, file_path: str) -> dict:
        ...
        async with get_connection(resolved_db) as conn:
            pid = await _project_id(conn, project_path)
            decisions = await repo.decisions_affecting_file(conn, file_path, project_id=pid)
        return {
            "file": file_path,
            "decisions": [
